...
export default function App() {
  ...
  // Monitora mudanças no sensor RFID (você precisará implementar esta parte)
  useEffect(() => {
    const sensorRef = collection(db, 'sensores');

    const q = query(sensorRef, where('status', '==', 'ativo'));

    const unsubscribe = onSnapshot(q, (snapshot) => {
      snapshot.docChanges().forEach((change) => {
        if (change.type === 'modified' || change.type === 'added') {
          const sensorData = change.doc.data();
          // Atualiza o setor atual baseado no sensor que detectou a tag
          setCurrentSector(sensorData.setor);
        }
      });
    }, (error) => {
      console.error("Erro ao monitorar sensores:", error);
    });
